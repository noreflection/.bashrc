#repositories
#alias r='cd ~/repositories'
alias r='cd C/Users/nshapovalov/repositories'
alias sd='cd ~/repositories/statistics/domain/'
alias sa='cd ~/repositories/statistics/api/'
alias p='cd c:/sourcecode2/postomat-control-center-nongitflow/'

alias lp="cd ~/Documents/'LINQPad Queries'/"

alias sc='cd c:/sourcecode/'
alias sc2='cd c:/sourcecode2/'
alias tv='cd c:/sourcecode/ozon.ui.tv/'
alias pd='cd c:/sourcecode/postomatdashboard'

#git
alias g='git'
alias ga='git add .'

alias gd='git diff'
alias gcl='git clone'
alias gcm='git commit -m'
alias gp='git push'
alias grv='git remote -v'

alias gs='git status'
alias gl='git log'
alias gst='git st'
alias glg='git lg'
alias gp='git push'

alias gb='git branch'
#alias gch='git checkout'

alias gcn='git config user.name'
alias gce='git config user.email'

#alias gk='gitk'
#alias ge='gitextensions'
alias gun='git config user.name'
alias gue='git config user.email'

#bash
alias c='cd'
alias cb='cd ..'
alias l='ls'
alias ~='cd ~'
#alias p='pwd'
alias ls='ls -a -1 --color -h --group-directories-first'
alias e='exit'
alias eb='code ~/.bashrc'
alias code='code .'

#dotnet
alias d='dotnet'
alias dr='dotnet restore'
alias db='dotnet build'
alias drun='dotnet run'
alias dn='dotnet new'

#yarn
alias y='yarn'
alias ys='yarn start'
alias yb='yarn build'
alias yt='yarn test'

#node
alias n='npm'
alias ns='npm start'
alias ni='npm install'
alias nr='npm run'

#frontend tools
alias cra='create-react-app'

#other
#windows specific
alias ex='explorer'
